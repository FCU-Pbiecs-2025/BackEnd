...
public class AdminHomeController {
  ...
  // 取得代辦事項數量 (status="待審核":聲請中, status="撤銷聲請中":撤銷聲請中)
  @GetMapping("/todo-counts")
  public ResponseEntity<Map<String, Integer>> getTodoCounts() {
    Integer count1 = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM dbo.application_participants WHERE Status = ?", Integer.class, "待審核");
    Integer count5 = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM dbo.application_participants WHERE Status = ?", Integer.class, "撤銷聲請中");
